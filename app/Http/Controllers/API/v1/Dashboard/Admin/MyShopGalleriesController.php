...
class MyShopGalleriesController extends AdminBaseController
{
    ...
    {
        $model = ShopGallery::where('shop_id', auth('sanctum')->user()?->shop?->id)->first();
    ...
    public function store(StoreRequest $request): JsonResponse
    {
        ...
        $validated['shop_id']   = auth('sanctum')->user()?->shop?->id;
    ...
    public function update(int $shopId, StoreRequest $request): JsonResponse
    {
        ...
        if ($model?->shop_id !== auth('sanctum')->user()?->shop?->id) {
    ...
    {
        $result = $this->service->deleteOne($id, auth('sanctum')->user()?->shop?->id);
