...
class ProductRepository extends CoreRepository implements ProductRepoInterface
{
     ...
     * @return LengthAwarePaginator
     */
    public function parentPaginate(array $filter): LengthAwarePaginator
    {
        /** @var Product $product */
        $product = $this->model();
        ...
        return $product
            ->filter($filter)
            ->whereNull('parent_id')
            ->when(isset($filter['not_in_ids']), function ($q) use ($filter) {
                $q->whereNotIn('id', $filter['not_in_ids']);
            })
            ->with([
     ...
     * @return mixed
     */
    public function productDetails(int $id): mixed
    {
        return $this->model()
     ...
     * @return mixed
     */
    public function productsByIDs(array $filter = []): mixed
    {
        return $this->model()
            ->filter($filter)
            ->with([
    ...
    public function selectStockPaginate(array $data): LengthAwarePaginator
    {
        return Stock::with([
            ...
            ->when(isset($data['addon']), fn($query, $addon) => $query->whereAddon($addon),
            ...
            ->whereHas('countable', fn($q) => $q->where('shop_id', data_get($data, 'shop_id') )
                ...
                ->when(isset($data['visibility']), fn($q) => $q->where('visibility', $data['visibility']))
