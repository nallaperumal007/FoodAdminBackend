...
class StoryRepository extends CoreRepository
{
     ...
     * @return mixed
     */
    public function index(array $filter = [], string $type = 'paginate'): mixed
    {
        /** @var Story $stories */
        $stories = $this->model();
        ...
        return $stories
            ...
            ->when(data_get($filter, 'shop_id'), fn(Builder $q, $shopId) => $q->where('shop_id', $shopId))
            ->when(data_get($filter, 'product_id'), fn(Builder $q, $productId) => $q->where('product_id', $productId))
...
//        $filter['status'] = 'approved';
        ...
        $shopsStories = Shop::filter($filter)->with([
            ...
            ->whereHas('products', function (Builder $query) use ($filter) {
                $query->when(data_get($filter, 'free'), function ($q) {
            ...
            ->when(data_get($filter, 'shop_id'), fn($q, $shopId) => $q->where('id', $shopId))
            ->select(['id', 'uuid', 'logo_img'])
            ->simplePaginate(data_get($filter, 'perPage', 100));
